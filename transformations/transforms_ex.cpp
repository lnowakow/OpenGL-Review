...
int main(int argc, char** argv) {
    ...
    // SHADER
    Shader ourShader("../shaders/transform_shader.glsl", "../shaders/fragment_shader_tex.glsl");
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        glm::mat4 transOverTime = glm::mat4(1.0f);
        // Remember, transforms are always read in reverse.  Here, result is first rotated, then translated.
        transOverTime = glm::translate(transOverTime, glm::vec3(0.5f, -0.5f, 0.0f));
        transOverTime = glm::rotate(transOverTime, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));

        unsigned int transformLoc = glGetUniformLocation(ourShader.ID, "transform");
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transOverTime));

        /*! bind the buffer and draw the shape you want... */
        glBindVertexArray(VAO);
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);

        /*! Configure how second element should be drawn */
        transOverTime = glm::mat4(1.0f);
        transOverTime = glm::translate(transOverTime, glm::vec3(-0.5f, 0.5f, 0.0f));
        float scaleAmount = sin(glfwGetTime());
        transOverTime = glm::scale(transOverTime, glm::vec3(scaleAmount, scaleAmount, scaleAmount));
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transOverTime));
